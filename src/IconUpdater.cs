...
public static class IconUpdater
{
    ...
    {
        Image icon = stats != null ? ArmorClassIconField.GetValue(stats) as Image : null;
    ...
    private static List<int> CollectClasses(Item item, IconDisplayMode mode)
    {
        if ((mode == IconDisplayMode.AllPlates || mode == IconDisplayMode.Custom)
            ...
            && holder.CompoundItem_0 != null)
        {
            var plates = CollectPlateClasses(holder.CompoundItem_0, mode == IconDisplayMode.Custom, out var hasPlates);
    ...
    private static List<int> CollectPlateClasses(CompoundItem item, bool filterByToggles, out bool hasPlates)
    {
        ...
        foreach (var slot in item.Slots.OfType<GClass3125>())
        foreach (var plate in slot.Items.OfType<ArmorPlateItemClass>())
        {
    ...
    private static void ShowDynamicBadges(GridItemView view, List<int> classes, bool examined)
    {
        ...
        {
            sprites[i] = CacheResourcesPopAbstractClass.Pop<Sprite>("Mod Types/icon_type_mod_armor_plate_" + classes[i]);
    ...
    private static bool IsEnabled(Item item)
    {
        return item switch
        {
            ...
            ArmoredEquipmentItemClass => Settings.EnableAccessories.Value,
            _ => false
        };
    }
}
